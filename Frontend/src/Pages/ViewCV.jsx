...
export default function ViewCV() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:3001/cvDetails/listCV-inprogress", {
        headers: {
  ...
  const handleViewClick = (cvId) => {
    navigate(`/viewEach-inprogress-cv/${cvId}`);
  };
      ...
      >
        {listOfCvs.length === 0 ? (
          <div style={{ textAlign: "center", marginTop: "20px" }}>
            <p style={{ color: "#b6b8b2" }}>No received CVs.</p>
          </div>
        ) : (
          listOfCvs.map((value) => (
            <div
